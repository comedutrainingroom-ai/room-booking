    ...
    return (
        <div className="border border-gray-100 rounded-2xl overflow-hidden transition-all hover:shadow-md">
            <button
                ...
                className={`w-full flex items-center gap-4 p-5 text-left transition-all ${isOpen ? 'bg-primary/5' : 'bg-white hover:bg-gray-50'}`}
            >
                <div className={`p-2.5 rounded-xl ${isOpen ? 'bg-primary text-white shadow-md' : 'bg-gray-100 text-gray-400'} transition-all`}>
                    {icon}
                </div>
                <span className="flex-1 font-bold text-gray-800">{title}</span>
                {isOpen ? <FaChevronUp className="text-primary" /> : <FaChevronDown className="text-gray-300" />}
            </button>
...
const StepItem = ({ number, title, description }) => (
    <div className="flex gap-4 items-start">
        <div className="w-8 h-8 rounded-full bg-primary text-white flex items-center justify-center font-bold text-sm flex-shrink-0 shadow-sm">
            {number}
        </div>
        <div>
    ...
    return (
        <div className="p-4 md:p-8 max-w-4xl mx-auto animate-fade-in">
            ...
            <div className="text-center mb-12">
                ...
                <h1 className="text-3xl font-bold text-gray-800 mb-2">คู่มือการใช้งานระบบ</h1>
                <p className="text-gray-500">แนะนำวิธีการใช้งานระบบจองห้องอบรมสำหรับผู้ใช้ทั่วไป</p>
            </div>

            {/* Quick Start Banner */}
            <div className="bg-gradient-to-r from-primary/10 to-emerald-50 rounded-2xl p-6 mb-8 border border-primary/20 flex items-start gap-4">
                <div className="p-3 rounded-xl bg-primary/10 text-primary text-xl flex-shrink-0">
                    <FaInfoCircle />
                </div>
                <div>
